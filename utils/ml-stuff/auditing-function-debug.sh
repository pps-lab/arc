...
cp  "$THE_CODE_DIR/scripts/auditing_function_debug.mpc" "$THE_CODE_DIR/mp-spdz/Programs/Source/custom-auditing_function_debug.mpc"
...
./compile.py -R 72 -C -D -M custom-auditing_function_debug "$6" split3
echo "End of compilation: $(date +\"%T.%N\")"
# Sleep for sync
sleep "$2"
# Execute mascot
./replicated-ring-party.x -h "$4" -pn 12300 "$1" custom-auditing_function_debug-$6-split3  | tee "$MY_CUR_DIR/results/result-$1.txt"
