...
impl MpcNet for MpcTwoNet {
    ...
    fn compute_global_data_sent() -> usize {
        let mut ch = get_ch!();
        let bytes_sent = ch.stats.bytes_sent.to_le_bytes();
        let result_all = Self::broadcast_bytes(&bytes_sent)
            .into_iter().map(|b| usize::from_le_bytes(b.try_into().unwrap()))
            .sum();

        // we have to subtract the total data sent here because we dont want it to count to our total statistics
        ch.stats.bytes_sent -= bytes_sent.len();
        ch.stats.bytes_recv -= bytes_sent.len();
        ch.stats.broadcasts -= 1;
